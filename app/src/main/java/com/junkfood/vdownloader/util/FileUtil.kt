...
private const val PRIVATE_DIRECTORY_SUFFIX = ".VDownloader"
...
object FileUtil {
    ...
    fun renameFile(oldPath: String, newTitle: String): Boolean {
        val oldFile = File(oldPath)
        val fileExtension = oldPath.substringAfterLast('.', "")
        val newPath = "${oldPath.substringBeforeLast('/')}/$newTitle.$fileExtension"
    ...
    internal fun getExternalDownloadDirectory() =
        File(Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS), "VDownloader")
            .also { it.mkdir() }
