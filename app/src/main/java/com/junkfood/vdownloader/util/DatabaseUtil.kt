...
import com.junkfood.vdownloader.App.Companion.applicationScope
import com.junkfood.vdownloader.App.Companion.context
import com.junkfood.vdownloader.database.AppDatabase
import com.junkfood.vdownloader.database.backup.Backup
import com.junkfood.vdownloader.database.backup.BackupUtil.BackupType
import com.junkfood.vdownloader.database.backup.BackupUtil.decodeToBackup
...
object DatabaseUtil {
    ...
    suspend fun updateVideoInfo(videoInfo: DownloadedVideoInfo, newTitle: String) {
        val newPath = "${videoInfo.videoPath.substringBeforeLast('/')}/$newTitle.${videoInfo.videoPath.substringAfterLast('.', "")}"
        if (FileUtil.renameFile(videoInfo.videoPath, newTitle)) {
            val updatedVideoInfo = videoInfo.copy(videoTitle = newTitle, videoPath = newPath)
            dao.updateVideoInfo(updatedVideoInfo)
        }
    }
